...
const String Version="Multimotor 3 axes V2.1 - 15-12-2025";

//******************
// V2.0 :changement de stratégie moteur : Appelé désormais X-Y-Z
// pour déclenchement de plusieurs moteur en même temps
// V2.1 : Débug sur réponse Série les Nb_Cmd sont gérer indépendamment des commentaires + Ajout Réponse commande VERSION
//******************
...
//VERSION : renvoi la version du code 
//*******************
...
void Lecture(){
      ...
      for (int i=0; i<=2 ; i++) {
        switch (Nb_Cmd[i]) {
          case -1 : 
            ...
            Serial.print(MoteurSelectionner[i]);
            Serial.println(" OFF");
            break;
          case 1 :
  ...
  if (Message.startsWith("ACCEL_")){
    ...
    if (Nb_Cmd[0]!=0){
      ...
      Serial.print("X VALID ACCEL = ");
      Serial.println(Nb_Cmd[0]);
    }
    if (Nb_Cmd[1]!=0){
      Moteur2.setAcceleration(Nb_Cmd[1]);
      Serial.print("Y VALID ACCEL = ");
      Serial.println(Nb_Cmd[1]);
    }
    if (Nb_Cmd[2]!=0){
      Moteur3.setAcceleration(Nb_Cmd[2]);
      Serial.print("Z VALID ACCEL = ");
      Serial.println(Nb_Cmd[2]);
    }
  } else {
  ...
  if (Message.startsWith("SPEED_")){
    ...
    if (Nb_Cmd[0]!=0){
      ...
      Serial.print("X VALID SPEED = ");
      Serial.println(Nb_Cmd[0]);
    }
    if (Nb_Cmd[1]!=0){
      Moteur2.setMaxSpeed(Nb_Cmd[1]);
      Serial.print("Y VALID SPEED = ");
      Serial.println(Nb_Cmd[1]);
    }
    if (Nb_Cmd[2]!=0){
      Moteur3.setMaxSpeed(Nb_Cmd[2]);
      Serial.print("Z VALID SPEED = ");
      Serial.println(Nb_Cmd[2]);
    }
  } else {
