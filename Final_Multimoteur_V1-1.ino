...
const String Version="Multimotor 3 axes V1.1 - 06-10-2025";
...
void GoHome(int Moteur){
    ...
    MoteurActif[Moteur].setAcceleration(5000);
    MoteurActif[Moteur].setSpeed(VitesseMax);
    //Avance rapide jusqu'à butée 
    while (digitalRead(LimitSwitchAvant[Moteur])==true){
      MoteurActif[Moteur].move(-10);
      while(MoteurActif[Moteur].distanceToGo()>0){
        MoteurActif[Moteur].run();
      }
    }
    //Retour arrière pour sortir de la butée
    while (digitalRead(LimitSwitchAvant[Moteur])==false){
      MoteurActif[Moteur].move(10);
      while(MoteurActif[Moteur].distanceToGo()>0){
        MoteurActif[Moteur].run();
      }
    ...
    MoteurActif[Moteur].setAcceleration(5000);
    MoteurActif[Moteur].setSpeed(VitesseMin);
    //Avance rapide jusqu'à butée 
    while (digitalRead(LimitSwitchAvant[Moteur])==true){
      MoteurActif[Moteur].move(-1);
      while(MoteurActif[Moteur].distanceToGo()>0){
        MoteurActif[Moteur].run();
      }
    }
    MoteurActif[Moteur].stop();
    MoteurActif[Moteur].setCurrentPosition(0);
